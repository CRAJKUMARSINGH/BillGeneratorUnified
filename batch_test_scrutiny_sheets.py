#!/usr/bin/env python3
"""
Batch Test Script for Scrutiny Sheet Generation
Processes all test input files and generates scrutiny sheets with performance metrics
"""
...
def format_test_result(filename, result, duration):
    ...
    if not result['success']:
        ...
        print(f"         Macro: {'✅' if result.get('macro_executed') else '❌'}")
        print(f"         Export: {'✅' if result.get('pdf_exported') else '❌'}")
...
def batch_process_scrutiny_sheets():
    """Batch process all test files for scrutiny sheet generation"""
    print("=" * 70)
    print("BATCH SCRUTINY SHEET GENERATION TEST")
    print("=" * 70)
    
    # Input and output directories
    input_dir = Path("TEST_INPUT_FILES")
    output_dir = Path("test_output") / "scrutiny_sheets"
    ...
    if test_file.exists():
        try:
            ...
            if not work_order_data.empty:
                print(f"\n📊 Work Order Data:")
                print(f"   Rows: {len(work_order_data)}")
                print(f"   Columns: {list(work_order_data.columns)}")
                print(f"   Sample rows:")
                print(work_order_data.head(3).to_string(index=False))
            
            # Extra items data
            extra_items_data = processed_data.get('extra_items_data', pd.DataFrame())
            if not extra_items_data.empty:
                print(f"\n➕ Extra Items Data:")
                print(f"   Rows: {len(extra_items_data)}")
                print(f"   Columns: {list(extra_items_data.columns)}")
                if 'Quantity' in extra_items_data.columns and 'Rate' in extra_items_data.columns:
                    total_extra = 0
                    for _, row in extra_items_data.iterrows():
                        try:
                            qty = float(row.get('Quantity', 0) or 0)
                            rate = float(row.get('Rate', 0) or 0)
                            total_extra += qty * rate
                        except:
                            pass
                    print(f"   Calculated Extra Items Total: ₹{total_extra:,.2f}")
...
if __name__ == "__main__":
    try:
        ...
        results = batch_process_scrutiny_sheets()
